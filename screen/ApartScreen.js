...
export default function ApartScreen({ navigation }) {
  ...
  const fetchApartments = async () => {
    try {
      const response = await fetch(`${BASE_URL}/apart`);
    ...
    >
      <View style={styles.cardContent}>
        <View style={styles.infoContainer}>
          ...
          <Text style={styles.rating}>{item.buildingArea}</Text>
          <Text style={styles.rating}>{item.buildingPrice}</Text>
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>Tidak ada building yang sedang tersedia</Text>
        }
      />
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  genres: { fontSize: 14, color: '#4CAF50', marginTop: 4 },
